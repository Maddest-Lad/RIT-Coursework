...
public class TestPieces {
    ...
    private static final String TEST_BOARD = TEST_DIR + "board13x13.txt";

    private Game game = new Game(TEST_BOARD);

    public TestPieces() throws FileNotFoundException {
    }
     ...
     * @param bad  A bunch of destinations from the final good position that should
     *             not work.
     */
    private void tryMoves(int[][] good, int[][] bad) {
        ...
        for (int move = 0; move < goodPath.length - 1; ++move)
        {
            ...
            assertTrue(this.game.makeMove(source, dest).ok,source + "-->" + dest);
        }
        ...
        {
            assertFalse(this.game.makeMove(lastGood, dest).ok,lastGood + "-->" + dest);
        }
    }
